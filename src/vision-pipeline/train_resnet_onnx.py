"""Обучение ResNet-50 по датасету из CVAT 
...
def train_resnet() -> None:
    dataset_root = Path(os.getenv("DATASET_ROOT", "/app/dataset_detection"))
    ckpt_out = Path(os.getenv("CKPT_OUT", "/app/models/resnet/resnet50_cls_22_06.onnx"))
    ...
    model.eval()
    dummy_input = torch.randn(1, 3, 224, 224, device=device_t)
    torch.onnx.export(
        model,
        dummy_input,
        str(ckpt_out),
    ...
    logger.info("Сохранили модель ResNet в ONNX: %s", ckpt_out)
    logger.info("Сохранили веса ResNet в %s", ckpt_out)

    import onnx
    onnx_model = onnx.load(str(ckpt_out))
    onnx.checker.check_model(onnx_model)
    logger.info("ONNX-модель прошла проверку валидности")
